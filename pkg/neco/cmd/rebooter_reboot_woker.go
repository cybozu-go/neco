...
RETRY:
	...
	kubernetesNodeAddrs := make(map[string]bool, len(nodes.Items))
	for _, node := range nodes.Items {
		kubernetesNodeAddrs[node.Name] = true
		if _, ok := machineAddrs[node.Name]; ok {
			kubernetesNodeMachine = append(kubernetesNodeMachine, node)
		}
	}

	for _, machine := range machines {
		if _, ok := kubernetesNodeAddrs[machine.Spec.IPv4[0]]; !ok {
