...
func runDraftCmd(cmd *cobra.Command, args []string) error {
	...
	prURL, err := createPR(branch, title, firstBody)
	if err != nil {
		return err
	}
...
func createPR(branch, title, body string) (string, error) {
	...
	return gc.CreatePullRequest(ctx, repoID, "master", branch, title, body)
}
