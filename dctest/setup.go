	...
	It("should complete on all boot servers", func(done Done) {
		...
		env.Go(func(ctx context.Context) error {
			stdout, stderr, err := execAt(
				boot0, "sudo", "env", "GOMAXPROCS=$(($(nproc) * 2))", "neco", "setup", "--no-revoke", "--proxy="+proxy, "0", "1", "2")
			if err != nil {
		...
		env.Go(func(ctx context.Context) error {
			stdout, stderr, err := execAt(
				boot1, "sudo", "env", "GOMAXPROCS=$(($(nproc) * 2))", "neco", "setup", "--no-revoke", "--proxy="+proxy, "0", "1", "2")
			if err != nil {
				log.Error("neco setup failed", map[string]interface{}{
					"host":   "boot-1",
					"stdout": string(stdout),
		...
		env.Go(func(ctx context.Context) error {
			stdout, stderr, err := execAt(
				boot2, "sudo", "env", "GOMAXPROCS=$(($(nproc) * 2))", "neco", "setup", "--no-revoke", "--proxy="+proxy, "0", "1", "2")
			if err != nil {
				log.Error("neco setup failed", map[string]interface{}{
					"host":   "boot-2",
					"stdout": string(stdout),
