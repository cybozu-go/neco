...
func TestCoilSetup() {
	It("should be deployed successfully", func() {
		...
		execSafeAt(bootServers[0], "ckecli", "etcd", "user-add", "coil", "/coil/")

		_, stderr, err := execAt(bootServers[0], "ckecli", "etcd", "issue", "coil", "--output", "file")
		Expect(err).ShouldNot(HaveOccurred(), "stderr=%s", stderr)

		_, stderr, err = execAt(bootServers[0], "kubectl", "--namespace=kube-system", "create", "secret",
		...
		Eventually(func() error {
			_, _, err := execAt(bootServers[0], "kubectl", "--namespace=kube-system", "get", "endpoints/cke-etcd")
			return err
		}).Should(Succeed())

		By("creating IP address pool")
		stdout, stderr, err := execAt(bootServers[0], "kubectl", "--namespace=kube-system", "get", "pods", "--selector=app.kubernetes.io/name=coil-controllers", "-o=json")
		...
		_, stderr, err = execAt(bootServers[0], "kubectl", "--namespace=kube-system", "exec", podName, "/coilctl", "pool", "create", "internet-egress", "172.19.0.0/28", "0")
		Expect(err).NotTo(HaveOccurred(), "stderr=%s", stderr)
		_, stderr, err = execAt(bootServers[0], "kubectl", "--namespace=kube-system", "exec", podName, "/coilctl", "pool", "create", "default", "10.64.0.0/14", "5")
		Expect(err).NotTo(HaveOccurred(), "stderr=%s", stderr)
	})
}
...
func TestCoil() {
	It("should be available", func() {
		...
		stdout, stderr, err = execAt(bootServers[0], "kubectl", "--namespace=kube-system", "exec", podName, "/coilctl", "pool", "list")
		Expect(err).NotTo(HaveOccurred(), "stderr=%s", stderr)
