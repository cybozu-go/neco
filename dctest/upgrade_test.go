...
func testUpgrade() {
	...
	It("should update cilium-agent", func() {
		podList := kubectlGetSafe[corev1.PodList](Default, "pods", "-n=kube-system", "-l=app.kubernetes.io/name=cilium-agent")
		...
		for _, img := range neco.CurrentArtifacts.Images {
			...
			Eventually(func() error {
				...
				case "squid-exporter":
					return checkVersionInDeployment("internet-egress", "squid", newImage)
				case "cilium":
					return checkVersionInDaemonSetPartial("kube-system", "cilium", newImage, 2)
