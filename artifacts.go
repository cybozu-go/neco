// Code generated by generate-artifacts. DO NOT EDIT.
//go:build !release

package neco

var CurrentArtifacts = ArtifactSet{
	Images: []ContainerImage{
		{Name: "coil", Repository: "ghcr.io/cybozu-go/coil", Tag: "2.4.0", Private: false},
		{Name: "bird", Repository: "ghcr.io/cybozu/bird", Tag: "2.14.0.1", Private: false},
		{Name: "chrony", Repository: "ghcr.io/cybozu/chrony", Tag: "4.4.0.2", Private: false},
		{Name: "etcd", Repository: "ghcr.io/cybozu/etcd", Tag: "3.5.10.2", Private: false},
		{Name: "promtail", Repository: "ghcr.io/cybozu/promtail", Tag: "2.9.2.2", Private: false},
		{Name: "sabakan", Repository: "quay.io/cybozu/sabakan", Tag: "3.0.0", Private: false},
		{Name: "serf", Repository: "ghcr.io/cybozu/serf", Tag: "0.10.1.4", Private: false},
		{Name: "setup-hw", Repository: "quay.io/cybozu/setup-hw", Tag: "1.15.2", Private: true},
		{Name: "squid", Repository: "ghcr.io/cybozu/squid", Tag: "6.6.0.1", Private: false},
		{Name: "squid-exporter", Repository: "ghcr.io/cybozu/squid-exporter", Tag: "1.0.2", Private: false},
		{Name: "vault", Repository: "ghcr.io/cybozu/vault", Tag: "1.15.3.2", Private: false},
		{Name: "cilium", Repository: "quay.io/cilium/cilium", Tag: "v1.14.7", Private: false},
		{Name: "cilium-operator-generic", Repository: "quay.io/cilium/operator-generic", Tag: "v1.14.7", Private: false},
		{Name: "hubble-relay", Repository: "quay.io/cilium/hubble-relay", Tag: "v1.14.7", Private: false},
		{Name: "cilium-certgen", Repository: "ghcr.io/cybozu/cilium-certgen", Tag: "0.1.9.2", Private: false},
	},
	Debs: []DebianPackage{
		{Name: "etcdpasswd", Owner: "cybozu-go", Repository: "etcdpasswd", Release: "v1.4.5"},
	},
	OSImage: OSImage{Channel: "stable", Version: "3602.2.3"},
}
